from superpixel import return_superpixels, pre_process, get_image_lut, show
...
if __name__ == "__main__":
    ...
    for i in adjacency:
        for j in adjacency[i]:
            ...
            g[i][j]['weight'] =  similarity_distance(color1, color2, d1, d2,di=51, dd=26000,r=70)
    """
    for (i, j) in g.edges:
        ...
        if res =='s':
            break
    """
    labels = graphcut_kmeans(graph=g, m=len(g.nodes), k=4)
    clusters = [list() for i in range(4)]
    ...
    for i in range(2):
        ...
        removes = clusters.pop(index)
        sub1 = g.copy()
        sub1.remove_nodes_from(removes)
        g = sub1
